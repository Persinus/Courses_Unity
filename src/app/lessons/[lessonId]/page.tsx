...
import { COURSE_STRUCTURE, type LessonContentBlock } from '@/lib/content';
...
export default function LessonPage({ params }: { params: { lessonId: string } }) {
  ...
  const renderContentBlock = (block: LessonContentBlock, index: number) => {
    switch (block.type) {
      case 'h2':
        return <h2 key={index} id={block.id} className="font-headline text-3xl font-bold mt-12 mb-6 scroll-mt-24">{block.text}</h2>;
      case 'h3':
        return <h3 key={index} id={block.id} className="font-headline text-2xl font-bold mt-8 mb-4 scroll-mt-24">{block.text}</h3>;
      case 'p':
  ...
  const lessonContentString = lesson.content.map(block => {
          ...
          <div className="flex-1 lg:grid lg:grid-cols-[1fr_240px] lg:gap-8">
            <main className="py-6 px-4 md:px-6 lg:px-8">
               <div className="prose prose-neutral dark:prose-invert max-w-none">
                <h1 className="font-headline text-4xl md:text-5xl font-extrabold tracking-tight mb-4">{lesson.title}</h1>
                <p className="text-lg text-muted-foreground mb-8">{lesson.summary}</p>
                ...
                <PracticeTasks
                    ...
                    lessonContent={lessonContentString}
